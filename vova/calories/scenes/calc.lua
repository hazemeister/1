...
activityText = display.newText(activityGroup, activity_name, display.contentCenterX, 790, "fonts/obelix", 18)
...
--Функция загрузки файла сохранения
function loadSetting(fileName)
	...
	local contents = "" --пустая переменная
	local myTable = {} --переменная таблица
	local file = io.open(path, "r") --открываем файл
	if(file) then --проверяем существует ли файл
		contents = file:read("*a") --читаем данные из файла
		myTable = json.decode(contents) --декодируем из json
		io.close(file) --закрываем файл(обязательно!)
		return myTable --возвращаем таблицу
	end
	return nil
end
-- -----------------------------------------------------------------------------------
